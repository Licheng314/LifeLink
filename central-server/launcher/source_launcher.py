"""Formal Windows entry point for a source checkout of the central service."""
...
def command_for() -> tuple[list[str], Path]:
    ...
    script = module_root() / "central_server_app.py"
    if python is None:
        raise RuntimeError("未找到 Python 无窗口解释器；请安装 Python 或设置 LIFE_LINK_SOURCE_PYTHON。")
    if not script.is_file():
        raise RuntimeError(f"未找到中央服务源码：{script}。请从完整项目目录启动。")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.validate:
            print("Life Link central source launcher is valid.")
