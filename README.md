...
- 基础环境：环境需要你安装 Python 3.13；首次运行会自动检查安装。
- 服务器：出于隐私因素，LifeLink完全需要你自行搭建服务器！建议安装在**长期运行AI伴侣**的个人电脑上。或者，你自己的服务器上。
- 网络连接：使用内网穿透+HTTPS地址来进行网络连接。如果你不知道怎么做，默认给你提供两种方案：
	- Tailscale：这个工具可以免费建立你的私有内网。代价：需要一点点安装配置流程，以及，它可能会跟你的手机VPN冲突。
	- 花生壳：只是多种可选的内网穿透的方案之一。我用它是因为它很方便——但是注意，这个要花一点钱！
- 如果只用本机PC客户端那就不用联网。
...
随后检查连接方式。每次手动运行这个 .BAT 都会执行一次连接流程，可以重新配置：

- Tailscale：执行下载安装Tailscale，保持 Tailscale 已登录，按提示完成。
- 花生壳：在花生壳的后台主页中找“内网穿透基础设置”，填写窗口显示的内网主机和内网端口，再把获得的 HTTPS 地址粘贴回 Life Link。

如果当前没有有效地址，则必须完成 Tailscale 或 HTTPS 内网穿透之一。
没有联网设置？那就只能用本机PC了。
...
但是，直接运行 EXE 和登录 Windows 后的自动启动不会弹出连接菜单；需要检查或调整连接时，需要再运行对应 .BAT。

### 3. 连接 Android

先下载安装 [最新 Android 测试版 APK](https://github.com/Licheng314/LifeLink/releases/latest/download/LifeLink-Android.apk)。
从中央服务托盘选择“生成设备配对码”，再到 Android 应用内输入。连接失败时依次确认手机网络、Tailscale 或内网穿透，以及中央服务是否正在运行。

### 4. 可选：连接 AI

打开你的PC客户端，从 WebUI 主页（中央服务器托盘图标上也行）生成“AI 配对包”，把压缩包交给支持 MCP 的 AI，AI有相应能力的话，它就能按包内说明完成连接。
