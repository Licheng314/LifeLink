...
SPEC = importlib.util.spec_from_file_location("pc_source_launcher_test", MODULE_PATH)
...
class SourceLauncherTests(unittest.TestCase):
    ...
    def test_configured_python_exe_uses_adjacent_pythonw(self) -> None:
        candidate = Path(r"C:\\Python313\\python.exe")
        self.assertEqual(launcher.windowless_candidate(candidate), Path(r"C:\\Python313\\pythonw.exe"))
    ...
    def test_pc_probe_requires_tkinter(self) -> None:
        candidate = Path(r"C:\\Python314\\pythonw.exe")
        with patch.object(Path, "is_file", return_value=True), patch.object(
            launcher.subprocess, "run"
        ) as run:
            run.return_value.returncode = 0
            self.assertTrue(launcher.is_supported_python(candidate))
            self.assertIn("tkinter", run.call_args.args[0][2])
