...
class TimelineDayPolicyTest {
    ...
    @Test
    fun queryWindowStartsAtCurrentBusinessDayBoundaryInShanghai() {
        val window = timelineDayWindow(dayStartHour, now)

        assertEquals(Instant.parse("2026-08-31T20:00:00Z"), window.fromInclusive)
        assertEquals(now, window.toExclusive)
    }

    @Test
    fun keepsOnlyCurrentBusinessDayInOriginalOrder() {
        val events = listOf(
        ...
        assertEquals(
            listOf("today-newer", "today-older"),
            currentBusinessDayTimelineEvents(events, dayStartHour, now).map { it.timelineEventId }
        )
    }

    @Test
    fun excludesEventsOutsideCurrentShanghaiBusinessDay() {
        val events = listOf(
    ...
    fun beforeBoundaryStillBelongsToThePreviousCalendarDatesBusinessDay() {
        ...
        assertEquals(
            listOf("today-before-boundary"),
            currentBusinessDayTimelineEvents(events, dayStartHour, beforeBoundaryNow).map { it.timelineEventId }
        )
    }
